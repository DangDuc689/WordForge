...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: words, error } = await client.from('vocabulary_items').select('id,english,vietnamese').eq('user_id', user.id).eq('status', 'active').limit(500)
    ...
    const known = (words ?? []).filter((word) => knownIds.has(word.id)).slice(0, 200)
    ...
    return json({ ...draft, english: term.trim(), tier: Math.min(3, Math.max(1, Number(draft.tier) || 1)), acceptedAnswers: Array.isArray(draft.acceptedAnswers) ? draft.acceptedAnswers.slice(0, 8) : [] })
